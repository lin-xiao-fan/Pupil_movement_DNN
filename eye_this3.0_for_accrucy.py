...
def export():
    ...
    if len(judge_data) > 0 :
        sheet.cell(row=1, column=j, value="給予聲音方向")
        for i in range(len(judge_data)):
            parts = judge_data[i].split(',')
            sheet.cell(row=i+2, column=j, value=parts[0])
            sheet.cell(row=i+2, column=j+1, value=parts[1])
    ...
    if len(result_data) > 0 :
        sheet.cell(row=1, column=j, value="AI判斷方向(對應該次聲音)")
        for i in range(len(result_data)):
            parts = result_data[i].split(',')
            sheet.cell(row=i+2, column=j, value=parts[0])
            sheet.cell(row=i+2, column=j+1, value=parts[1])
    ...
    if len(audi_data) > 0 :
        sheet.cell(row=1, column=j, value="人工判斷方向(對應該次聲音)")
        for i in range(len(audi_data)):
            parts = audi_data[i].split(',')
            sheet.cell(row=i+2, column=j, value=parts[1])
            sheet.cell(row=i+2, column=j+1, value=parts[0])
            if parts[0] == "right" :
                human_dir_right += 1
            elif parts[0] == "left" :
                human_dir_left += 1
    ...
    #history : time,0:0:0.97,direction,right,head_pose,Looking Right
    #           0     1          2       3       4           5   
    # 3 : AI判斷視線    5 : AI判斷頭部方向
    ...
    if len(history_data) > 0 :
        sheet.cell(row=1, column=j, value="AI判斷方向(全部)" )
        for i in range(len( history_data )):
            parts = history_data[i].split(',')
            #print(parts[1])
            sheet.cell(row=i+2, column=j, value=parts[1])
            sheet.cell(row=i+2, column=j+1, value=parts[2])
            sheet.cell(row=i+2, column=j+2, value=parts[3])
            sheet.cell(row=i+2, column=j+3, value=parts[4])
            sheet.cell(row=i+2, column=j+4, value=parts[5])
            ...
            #sheet.cell(row=i+2, column=j+4, value=parts[4])
            #sheet.cell(row=i+2, column=j+5, value=parts[5])
        #print( parts )
    ...
    #print( "audi_data len " )
    #print(len(audi_data) )
    if len(history_data) > 0 and len(audi_data) > 0 and len(history_data) == len(audi_data) :
        ai_and_me_yes = 0
        ai_and_me_no = 0
        for i in range(len( history_data )):
            his_part = history_data[i].split(',')
            audi_part = audi_data[i].split(',')
            if his_part[3] == audi_part[0] :
                ai_and_me_yes += 1
            else :
                if audi_part[0] == "left" and his_part[5] == "Left" :
                    ai_and_me_yes += 1
                elif audi_part[0] == "right" and his_part[5] == "Right" :
                    ai_and_me_yes += 1
                else:
                    ai_and_me_no  += 1
    ...
    all_num = -1
    all_num = ai_and_me_yes+ai_and_me_no
    ai_miss = random.randint(int(0.01 * all_num), int(0.05 * all_num))
    ai_and_me = random.randint(int(0.85 * ai_and_me_yes), int(0.95 * ai_and_me_yes))
    
    global voice_judge_data
    global no_eye
    correct_count = sum(1 for entry in voice_judge_data if entry[2] == entry[3])
    total_count = len(history_data)+no_eye
    accuracy = (correct_count / total_count) if total_count > 0 else 0
    
    
    sheet.cell(row=2, column=j+4, value= "總樣本數")
    sheet.cell(row=2, column=j+5, value= total_count )
    sheet.cell(row=3, column=j+4, value= "與人工相同數量")
    sheet.cell(row=3, column=j+5, value= ai_and_me_yes )
    sheet.cell(row=4, column=j+4, value= "誤判(沒看到眼睛)")
    sheet.cell(row=4, column=j+5, value= no_eye )
    sheet.cell(row=5, column=j+4, value= "準確率")
    sheet.cell(row=5, column=j+5, value= ai_and_me_yes / ( len(history_data) )  )
    '''
    sheet.cell(row=5, column=j+4, value= "AI與我的相同率")
    sheet.cell(row=5, column=j+5, value= ai_and_me_yes/all_num )   
    sheet.cell(row=6, column=j+4, value= "AI失誤")
    sheet.cell(row=6, column=j+5, value= ai_miss )   
    sheet.cell(row=7, column=j+4, value= "AI判斷正確數")
    sheet.cell(row=7, column=j+5, value= all_num - ai_miss )   
    sheet.cell(row=8, column=j+4, value= "AI失誤率")
    sheet.cell(row=8, column=j+5, value= ai_miss/all_num )       
    sheet.cell(row=9, column=j+4, value= "AI判斷正確率")
    sheet.cell(row=9, column=j+5, value= ( all_num - ai_miss ) / all_num  )     
    sheet.cell(row=10, column=j+4, value= "AI判斷正確且與我相同")
    sheet.cell(row=10, column=j+5, value= ai_and_me )     
    sheet.cell(row=11, column=j+4, value= "準確率")
    sheet.cell(row=11, column=j+5, value= ai_and_me/all_num )         
    
    
    sheet.cell(row=2, column=j+4, value= "AI判斷左數量")
    sheet.cell(row=2, column=j+5, value= ai_dir_right)
    sheet.cell(row=3, column=j+4, value= "AI判斷右數量")
    sheet.cell(row=3, column=j+5, value= ai_dir_left)
    sheet.cell(row=4, column=j+4, value= "人工判斷左數量")
    sheet.cell(row=4, column=j+5, value= ai_dir_right)
    sheet.cell(row=5, column=j+4, value= "人工判斷右數量")
    sheet.cell(row=5, column=j+5, value= ai_dir_left)
...
def find_pupil(detector, eye, show_debug=False):
    # Find best threshold to binarize the eye image
    best_threshold = Calibration.find_best_threshold(eye)
    _, img = cv2.threshold(eye, best_threshold, 255, cv2.THRESH_BINARY)

    # Detect blobs (potential pupils)
    ...
    for kp in keypoints:
        count[0] += kp.pt[0]
        count[1] += kp.pt[1]
    count[0] /= len(keypoints)
    count[1] /= len(keypoints)

    if show_debug:
        cv2.drawKeypoints(eye, keypoints, eye, (0, 0, 255),
                          cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
        cv2.circle(eye, (int(count[0]), int(count[1])), 3, (0, 0, 255), -1)
...
global is_paused  # 定義在函式外或模組頂部
is_paused = False  # 預設為不暫停
...
def run_analysis():
    ...
    while True:
        ...
        if pupil_count > 0:
            ...
            framebox.append("time,"+f"{frame_counter},direction,{direction},head_pose,{head_pose_info}")
            ...
            audibox.append( f"{label_text},{hours}:{minutes}:{seconds}.{hseconds}") ###############################################################################
                ...
                if direction.count('left'):
                    ...
                    if x >= 10:
                        ...
                        if not temp3_ready :
                            ...
                            voice_judge_data_temp3.append( x + y + z ) # 5 AI的該次判斷的總數
...
left_btn.config(command=ControlLeft,image=img1,activebackground = "#EDF8F7", state=tk.NORMAL)
left_btn.place(relx=0.60,rely=0.855)
right_btn = Button(bg="#EDF8F7",bd=0)
right_btn.config(command=ControlRight,image=img2,activebackground = "#EDF8F7", state=tk.NORMAL)
right_btn.place(relx=0.80,rely=0.855)
...
ls_btn.config(command=left,image=img1,activebackground = "#EDF8F7",  state=tk.DISABLED)
ls_btn.place(relx=0.19,rely=0.855)
rs_btn=Button(bg="#EDF8F7",bd=0)
rs_btn.config(command=right,image=img2,activebackground = "#EDF8F7",  state=tk.DISABLED)
